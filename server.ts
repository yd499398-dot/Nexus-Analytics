...
import { analyzeAttrition, _modelInfo } from './ml/attritionModel';
import helmet from 'helmet';
import rateLimit from 'express-rate-limit';

// Local ML model (Random Forest of decision trees) is trained in-process
// when ./ml/attritionModel is imported above — no API key, no network call.
console.log(`[ML] Local attrition model ready:`, _modelInfo());
...
app.post('/api/analyze-attrition', async (req, res) => {
  try {
    const { salary, commute, satisfaction, tenure, overTime, jobInvolvement, lang, emailVolumeDecline, emailAfterHours, emailSentimentRisk, emailResponseDelay } = req.body;

    // Local prediction — runs the in-process Random Forest, no network call.
    const result = analyzeAttrition(
      { salary, commute, satisfaction, tenure, overTime, jobInvolvement, emailVolumeDecline, emailAfterHours, emailSentimentRisk, emailResponseDelay },
      lang
    );
