...
export default function DashboardLayout() {
  ...
  return (
    <div className={`h-dvh ${darkMode ? 'bg-slate-900' : 'bg-slate-50'} flex overflow-hidden selection:bg-indigo-500/30`}>
      ...
      {/* Sidebar - fixed to the viewport on md+ so it can never scroll/slide with page content, even on mobile browsers that resize their address bar. */}
      <aside 
        className={`w-64 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} border-r flex-col hidden md:flex md:fixed md:inset-y-0 md:left-0 shrink-0 z-20 shadow-sm h-dvh overflow-y-auto custom-scrollbar`}
      ...
      <main 
        id="main-content"
        tabIndex={-1}
        className={`flex-1 flex flex-col min-w-0 overflow-hidden md:ml-64 ${darkMode ? 'bg-slate-900' : 'bg-slate-50'} relative outline-none`}
      >
