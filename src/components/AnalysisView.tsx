...
import { Hexagon, Users, GitBranch, Loader2, FileText, Download, Copy, Check, X } from 'lucide-react';
import { AnalysisResult, EmployeeData } from '../types';
import { useLanguage } from './LanguageContext';
import DecisionPathTree from './DecisionPathTree';
    ...
    <div className="flex flex-col space-y-4 pb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <motion.div 
          ...
          className="lg:col-span-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-5 flex flex-col lg:sticky lg:top-4 lg:self-start"
        >
        ...
        {/* Center Column: Decision Path Tree */}
        <div className="lg:col-span-9 flex flex-col space-y-4">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col relative"
          >
            <div className="flex items-center justify-between px-5 pt-5 pb-3 shrink-0 border-b border-slate-100 dark:border-slate-700/60">
              <div className="flex items-center space-x-3 text-slate-900 dark:text-slate-100">
                ...
                <div>
                  <h2 className="font-display font-semibold text-sm">Attrition Decision Tree</h2>
                  <p className="text-[11px] text-slate-500 mt-0.5">Tap a node to inspect that decision</p>
                </div>
              </div>
              {result?.riskLevel && (
                <div className={`text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full border ${
                  result.riskLevel === 'High' ? 'bg-rose-50 text-rose-700 border-rose-100 dark:bg-rose-950/40 dark:text-rose-400 dark:border-rose-900/50' :
                  result.riskLevel === 'Medium' ? 'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-950/40 dark:text-amber-400 dark:border-amber-900/50' :
